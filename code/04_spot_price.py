...
spot_prices = requests.get(api_url, params=params, headers={"X-Params": json.dumps(header)})
...
if spot_prices.status_code == 200:
    ...
    for data_point in spot_price_series:
        ...
        spot_data_values.append({
            'variable': var, 
            'date': date, 
            'spot price (nominal)': spot_price_nom
        })
...
spot_prices_df = spot_prices_df.pivot(index='date', columns='variable', values='spot price (nominal)').reset_index()
print(spot_prices_df)
# I need to make both the US and CA float-type variables 
columns_to_convert = ["EER_EPMRR_PF4_Y05LA_DPG", "EER_EPMRU_PF4_RGC_DPG", "EER_EPMRU_PF4_Y35NY_DPG", "RBRTE"]
...
spot_prices_df = spot_prices_df.rename(columns={
...
spot_prices_df.to_csv('/Users/danpost/Dropbox/Myster_Gas_Surcharge/Analysis/Data/spot_prices.csv', index=False)