...
gas_tax_dot_df = gas_tax_dot_df.rename(columns={
    'MMFR_year': 'date',
    'rate': 'gas excise tax'
})
...
gas_sales_dot_df['date'] = pd.to_datetime(gas_sales_dot_df['date'], format='%m/%d/%y %H:%M')
...
gas_sales_tax_dot_df = pd.merge(gas_tax_dot_df, gas_sales_dot_df, on=['state','date'], how='outer')
...
avg_gas_state_tax_df['gas excise tax'] = avg_gas_state_tax_df['gas excise tax'].fillna(method='ffill')
    # now I'm going to calculate the average tax rate for all states, grouped by date and weighted by their gasoline sold
avg_gas_state_tax_df = avg_gas_state_tax_df.groupby('date').apply(lambda x: (x['gas excise tax'] * x['value']).sum() / x['value'].sum())
    # reset index
avg_gas_state_tax_df = avg_gas_state_tax_df.reset_index(name='average state tax excl. ca')
...
gas_sales_dot_filtered_df = gas_sales_dot_df[gas_sales_dot_df['state'].isin(states_of_interest)]
...
gas_sales_dot_filtered_df = gas_sales_dot_filtered_df.rename(columns={
    'California': 'ca total gas sold',
    'US Total': 'usa total gas sold'
})
# generate a variable demonstrating the share of gas sold in the country sold in california
gas_sales_dot_filtered_df['ca share of usa gas'] = gas_sales_dot_filtered_df['ca total gas sold']/gas_sales_dot_filtered_df['usa total gas sold']
...
gas_fed_ca_tax_df = gas_fed_ca_tax_df.pivot_table(index='date', 
    columns='state', 
    values='gas excise tax', 
    fill_value=None
)
# rename appropriately
gas_fed_ca_tax_df = gas_fed_ca_tax_df.rename(columns={
    'California': 'ca state gas tax',
    'Federal Tax': 'federal gas tax'
})
...
gas_tax_df[['ca state gas tax', 'federal gas tax']] = gas_tax_df[['ca state gas tax', 'federal gas tax']].fillna(method='ffill')
