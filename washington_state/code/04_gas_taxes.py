...
data = (work_dir / 'data')
raw_data_taxes = (home_dir / 'mystery_ca_gas_surcharge' / 'data' / 'raw')
...
wa_excise_df = gas_tax_df[gas_tax_df['state'] == 'Washington']
wa_excise_df.rename(
    columns = {
        'gas excise tax': 'wa excise tax'
    }, inplace=True
)
wa_excise_df = wa_excise_df[['wa excise tax', 'date']]

states_to_remove = ['US Total', 'Federal Tax', 'Washington']
...
avg_gas_state_tax_df = avg_gas_state_tax_df.reset_index(
    name='average state tax excl. wa'
)

# the next thing we need is California's share of the us total gasoline sold
states_of_interest = ['US Total', 'Washington']
wa_share_usa_total = gas_sales_df[
...
wa_share_usa_total = wa_share_usa_total.pivot_table(
    index='date', columns='state', values='gasoline sales', fill_value=None
)
wa_share_usa_total = wa_share_usa_total.reset_index()
wa_share_usa_total = wa_share_usa_total.rename(
    columns={'Washington': 'wa total gas sold', 'US Total': 'usa total gas sold'}
)
# generate a variable demonstrating the share of gas sold in the country sold in california
wa_share_usa_total['wa share of usa gas'] = (
    wa_share_usa_total['wa total gas sold']
    / wa_share_usa_total['usa total gas sold']
)
...
wa_share_usa_total['wa share of usa gas'].fillna(0.1, inplace=True)
# now drop the total variables, we're not interested at all
cols_drop = ['usa total gas sold']
wa_share_usa_total = wa_share_usa_total.drop(columns=cols_drop)

# now merge our 3 datasets of interest:
gas_tax_final_df = pd.merge(wa_share_usa_total, avg_gas_state_tax_df, on='date', how='outer')
...
plt.plot(gas_tax_final_df['date'], gas_tax_final_df['wa excise tax'],
         label = 'WA Excise Tax', color='blue')
plt.plot(gas_tax_final_df['date'], gas_tax_final_df['average state tax excl. wa'],
         label = 'Average National Tax Excl. WA', color='orange')
plt.legend()
plt.show()
