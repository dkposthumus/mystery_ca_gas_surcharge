...
or_excise_df = gas_tax_df[gas_tax_df['state'] == 'Oregon']
or_excise_df.rename(
    columns = {
        'gas excise tax': 'or excise tax'
    }, inplace=True
)
or_excise_df = or_excise_df[['or excise tax', 'date']]

states_to_remove = ['US Total', 'Federal Tax', 'Oregon']
...
avg_gas_state_tax_df = avg_gas_state_tax_df.reset_index(
    name='average state tax excl. or'
)

# the next thing we need is California's share of the us total gasoline sold
states_of_interest = ['US Total', 'Oregon']
or_share_usa_total = gas_sales_df[
...
or_share_usa_total = or_share_usa_total.pivot_table(
    index='date', columns='state', values='gasoline sales', fill_value=None
)
or_share_usa_total = or_share_usa_total.reset_index()
or_share_usa_total = or_share_usa_total.rename(
    columns={'Oregon': 'or total gas sold', 'US Total': 'usa total gas sold'}
)
# generate a variable demonstrating the share of gas sold in the country sold in california
or_share_usa_total['or share of usa gas'] = (
    or_share_usa_total['or total gas sold']
    / or_share_usa_total['usa total gas sold']
)
# now drop the total variables, we're not interested at all
cols_drop = ['usa total gas sold']
or_share_usa_total = or_share_usa_total.drop(columns=cols_drop)

# now merge our 3 datasets of interest:
gas_tax_final_df = pd.merge(or_share_usa_total, avg_gas_state_tax_df, on='date', how='outer')
gas_tax_final_df = pd.merge(gas_tax_final_df, or_excise_df, on='date', how='outer')
...
plt.plot(gas_tax_final_df['date'], gas_tax_final_df['or excise tax'],
         label = 'OR Excise Tax', color='blue')
plt.plot(gas_tax_final_df['date'], gas_tax_final_df['average state tax excl. or'],
         label = 'Average National Tax Excl. OR', color='orange')
plt.legend()
plt.show()
